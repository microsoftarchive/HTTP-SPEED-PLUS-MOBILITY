...
function SMSession(address, options) {
	...
	function serializeAndSend(frame) {
		var data = self._framer.serialize(frame);
		self._socket.send(data, {binary:true, fin:false});
		if (frame.fin) {
	...
	function handleWebSocketClose(code, message) {	
		...
		if (self.state != SMSession.CLOSED){
			...
			self.emit('close', Error.PROTOCOL_ERROR, message);
		}
	}
	...
	function handleFrameError(frame, statusCode) {
		if (frame.streamId) {
			...
			if (self._streams[frame.streamId]) closeStream(self._streams[frame.streamId]);	
	...
	function handleWebSocketMessage(data) {		
		try {
			self.emit.apply(this, arguments);
			var frame = self._framer.deserialize(data);
			processFrame(frame);
		} catch (error) {
